...
public class Moves {
     ...
     */
    public static ArrayList<ArrayList<Integer>> generateSlidingMoves (int startingIndex, Board board) { // Account for pawns
        ...
        for (int directionIndex = startingDirIndex; directionIndex < endingDirIndex; directionIndex ++) {
            for (int n = 0; n < Pieces.moveData(startingIndex)[directionIndex]; n++) {
                ...
                if (Pieces.sameColor(board.getPosition().get(targetIndex), colorUp)) {
                    break;
                }
